...
import "./IERC4626Mod.sol";

abstract contract ERC4626MOD is ERC20, IERC4626MOD {
    using Math for uint256;

    uint internal assetBalances;
    ...
    function deposit(uint256 assets, address receiver) public payable virtual returns (uint256) {
        ...
        require(msg.value >= assets,"incorr amount");
    ...
    function _withdraw(
        ...
        _burn(owner, shares);
        bool z = payable (receiver).send(assets);
        if(!z) revert("failed");
        assetBalances -= assets;
